...
async getVisibleHomeLogo(): Promise<ElementHandle<Element> | undefined> {
  for (let i = 0; i < this.maxRetries; i++) {
    ...
    for (let j = 0; j < count; j++) {
      ...
      const isValid = await candidate.evaluate((el) => {
    ...
    if (validCandidates.length > 0) {
      ...
      console.log(`[Try ${i + 1}] Randomly selected visible logo at index ${randomIndex}`);
    ...
    console.log(`[Try ${i + 1}] No matching logos found. Waiting and retrying...`);
    await this.page.waitForTimeout(300);
  }

  console.warn(`[FAILED] Tensor.Art logo was not visible after ${this.maxRetries} attempts.`);
  return undefined;
}
  ...
  async forceGoToHome(maxRetries = 10): Promise<void> {
    for (let i = 0; i < maxRetries; i++) {
      ...
      if (!backExists) {
        console.log(
          `Back to Home disappeared - assuming homepage is loaded.`
        );
        expect(true).toBe(true);
        return;
      }
